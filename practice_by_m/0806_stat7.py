...
result = ttest_1samp(tab1["score"], popmean=10, alternative='two-sided') # two-side 기본옵션- 삭제 가능 
...
result.pvalue
result.df # 자유도
# 귀무가설이 참(mu = 10)일때, 11.53(표본평균)이 관찰될 확률이 6.48(pvalue) 이므로 
# 이것은 우리가 생각하는 보기 힘들다고 판단하는 기준인 
# 0.05(유의수준)보다 크므로, 귀무가설을 거짓이라 판단하기 힘들다.

# 유의확률 0.0648이 유의수준 0.05 보다 크므로
# 귀무가설을 기각하지 못한다.

 ## p_value가 유의수준보다 크면 귀무가설 기각 못하고 작으면 기각
...
# 첫번째 입력그룹의 평균이 두번째 입력 그룹평균보다 작다.
# 고 설정된 경우를 나타냄 
